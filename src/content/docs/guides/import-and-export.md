...
| `.eplus` | A set of content, packed as an archive with its images beside it |
...
`.eplus` is the universal Encounter+ archive: content as JSON, with its images stored beside it.
It is what an export of loose content produces, and it imports back the same way.

:::tip
`.module`, `.campaign`, `.system` and `.eplus` are all the same kind of archive — what is inside
decides how it is imported, not the extension. See [File Types](/reference/file-types/).
:::
...
Yes. The current formats are JSON, so an `.eplus` archive — or any `.module`, `.campaign` or
`.system` — can be edited by hand or generated by a script. See the
[schema reference](/reference/schema/) and [File Types](/reference/file-types/).
